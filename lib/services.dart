...
Future<AllSpecies> getAllSpecies(String type) async {
  final responce = await http.get('$url/species/category/$type?$token');
  print(responce.body);
  return allSpeciesFromJson(responce.body);
}

Future<String> getLink(String name) async {
  final responce = await http.get('$url/weblink/$name');
  print(responce.body);
...
Future<Specie> getSpecie(int id) async {
  final responce = await http.get('$url/species/narrative/id/$id?$token');
  print(responce.body);
